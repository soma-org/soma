use crate::error::{ShardError, ShardResult, SharedResult};
...
pub struct ActorMessage<P: Processor> {
    pub input: P::Input,
    pub sender: oneshot::Sender<ShardResult<P::Output>>,
...
mod tests {
    ...
    async fn test_background_processing() {
        ...
        handle
            .background_process(5, CancellationToken::new())
            .await
            .unwrap();
    ...
    async fn test_handle_clone() {
        ...
        let result2 = handle_clone
            .process(6, CancellationToken::new())
            .await
            .unwrap();
