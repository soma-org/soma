...
impl AuthenticatorTrait for MultiSig {
        ...
        T: Serialize,
    {
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            ...
            let res = match sig {
                CompressedSignature::Ed25519(s) => {
                    ...
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| SomaError::InvalidSignature {
        ...
        if weight_sum >= self.multisig_pk.threshold {
            ...
            Err(SomaError::InvalidSignature {
                error: format!(
...
impl MultiSig {
    ...
    pub fn get_indices(&self) -> Result<Vec<u8>, SomaError> {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| SomaError::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| SomaError::InvalidSignature {
