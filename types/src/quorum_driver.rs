...
pub enum QuorumDriverError {
    ...
    #[error(
        "Failed to sign transaction by a quorum of validators because of locked objects: {:?}, \
         retried a conflicting transaction {:?}, success: {:?}",
        conflicting_txes,
        retried_tx,
        retried_tx_success
    )]
    ...
    #[error(
        "Transaction failed to reach finality with transient error after {total_attempts} \
         attempts."
    )]
    ...
    #[error(
        "Transaction is not processed because {overloaded_stake} of validators by stake are \
         overloaded with certificates pending execution."
    )]
    SystemOverload {
    ...
    #[error(
        "Transaction is not processed because {overload_stake} of validators are overloaded and \
         asked client to retry after {retry_after_secs}."
    )]
