...
mod model_tests {
    ...
    use crate::system_state::test_utils::{
        ...
        commit_model, create_model, create_model_with_commission, create_test_system_state,
        create_validator_for_testing, reveal_model, stake_with_model, unstake,
    };
    ...
    fn test_model_commit_reveal() {
        ...
        // Epoch 0: Create model (economic setup)
        let staked = create_model(&mut state, model_owner(), model_id_1(), 5 * SHANNONS_PER_SOMA);

        // Model should be in Created state
        let model = state.model_registry().models.get(&model_id_1()).unwrap();
        assert!(model.is_created());
        assert!(!model.is_active());

        // Epoch 0: Commit model (transitions Created -> Pending)
        commit_model(&mut state, model_owner(), &model_id_1());

        // Model should now be in Pending state
        let model = state.model_registry().models.get(&model_id_1()).unwrap();
        ...
        // Model should now be Active
        let model = state.model_registry().models.get(&model_id_1()).unwrap();
        assert!(model.is_active());
        assert!(!model.is_pending());
        let active = model.as_active().unwrap();
        assert!(active.decryption_key != crate::crypto::DecryptionKey::new([0u8; 32]));
        assert_eq!(active.staking_pool.soma_balance, 5 * SHANNONS_PER_SOMA);
    ...
    fn test_model_commit_no_reveal_slash() {
        ...
        // Epoch 0: Create + Commit model
        let staked = create_model(&mut state, model_owner(), model_id_1(), stake);
        commit_model(&mut state, model_owner(), &model_id_1());
        assert!(state.model_registry().models.get(&model_id_1()).unwrap().is_pending());
    ...
    fn test_model_staking_delegation() {
        ...
        // Epoch 0: Create + Commit model
        let _owner_staked = create_model(&mut state, model_owner(), model_id_1(), initial_stake);
        commit_model(&mut state, model_owner(), &model_id_1());
        ...
        let active = state.model_registry().models.get(&model_id_1()).unwrap().as_active().unwrap();
        assert_eq!(active.staking_pool.soma_balance, initial_stake + 20 * SHANNONS_PER_SOMA);
        assert_eq!(
        ...
        let active = state.model_registry().models.get(&model_id_1()).unwrap().as_active().unwrap();
        assert_eq!(active.staking_pool.soma_balance, initial_stake);
    ...
    fn test_model_min_stake_validation() {
        ...
        // But we can verify that creating with the min stake works...
        let staked = create_model(&mut state, model_owner(), model_id_1(), min_stake);
        assert!(state.model_registry().models.get(&model_id_1()).unwrap().is_created());

        // ...and that create_model itself accepts exactly model_min_stake
    ...
    fn test_model_architecture_validation() {
        ...
        // request_create_model does NOT validate architecture_version — the executor does.
        // The SystemState method just stores what it's given. So we verify the stored version.
        let result = state.request_create_model(
            model_owner(),
            model_id_1(),
            wrong_version,
            stake,
    ...
    fn test_model_update_commit_reveal() {
        ...
        // Create + Commit + reveal the model
        let _staked = create_model(&mut state, model_owner(), model_id_1(), stake);
        commit_model(&mut state, model_owner(), &model_id_1());
        let mut vr = ValidatorRewards::new(&state.validators().validators);
        advance_epoch_with_reward_amounts(&mut state, 0, &mut vr);
        reveal_model(&mut state, model_owner(), &model_id_1());

        // Commit model update (unified — works on Active models)
        commit_model(&mut state, model_owner(), &model_id_1());
        ...
        // Reveal model update (unified — works on Active models with pending_update)
        reveal_model(&mut state, model_owner(), &model_id_1());

        let model = state.model_registry().models.get(&model_id_1()).unwrap();
        assert!(!model.has_pending_update());
        // Model should still be active after update
        assert!(model.is_active());
    }
    ...
    fn test_model_update_no_reveal_cancel() {
        ...
        // Commit update (unified — sets pending_update on Active)
        commit_model(&mut state, model_owner(), &model_id_1());
        assert!(state.model_registry().models.get(&model_id_1()).unwrap().has_pending_update());
    ...
    fn test_model_update_overwrite() {
        ...
        // First commit update (unified — works on Active)
        commit_model(&mut state, model_owner(), &model_id_1());
    ...
    fn test_model_set_commission_rate() {
        ...
        let active = state.model_registry().models.get(&model_id_1()).unwrap().as_active().unwrap();
        assert_eq!(active.commission_rate, 0);
        assert_eq!(active.next_epoch_commission_rate, 1000);

        // Advance epoch to apply
        advance_epoch_with_reward_amounts(&mut state, 0, &mut vr);

        let active = state.model_registry().models.get(&model_id_1()).unwrap().as_active().unwrap();
        assert_eq!(active.commission_rate, 1000);
        assert_eq!(active.next_epoch_commission_rate, 1000);
    }
    ...
    fn test_model_commission_rate_too_high() {
        ...
        // Create with rate > BPS_DENOMINATOR should fail
        let staking_pool_id = ObjectID::random();
        ...
        let _staked = create_model(&mut state, model_owner(), model_id_2(), stake);
        commit_model(&mut state, model_owner(), &model_id_2());
        let mut vr = ValidatorRewards::new(&state.validators().validators);
        advance_epoch_with_reward_amounts(&mut state, 0, &mut vr);
    ...
    fn test_model_report_quorum_removal() {
        ...
        assert!(state.model_registry().models.get(&model_id_1()).unwrap().is_active());
    ...
    fn test_model_not_owner_errors() {
        ...
        // Non-owner tries to commit update (unified — on Active model)
        ...
        let result =
            state.request_commit_model(not_owner, &model_id_1(), manifest2, wc2, ec2, dkc2);
        match result {
            Err(ExecutionFailureStatus::NotModelOwner) => {}
            other => panic!("Expected NotModelOwner, got {:?}", other),
        }
