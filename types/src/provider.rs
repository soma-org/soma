...
//! object; `UpdateProvider` doubles as a heartbeat (the executor stamps
//! `registered_at_ms` on every call) and as the way to change the
//! advertised endpoint.
//!
...
///
/// Minimum-viable layout: address, endpoint, timestamp. Anything
/// else (`supported_models`, stake, `pubkey_hex`, region, SKU tags)
/// is deliberately omitted — those don't change behavior at this
/// stage and can be added without a layout change later.
...
pub struct Provider {
    ...
    /// Wall-clock time (from the consensus Clock) at which the most
    /// recent `RegisterProvider` / `UpdateProvider` landed. Off-chain
    /// liveness logic uses this as a heartbeat: a provider whose
    /// `registered_at_ms` is older than some staleness threshold is
    /// dropped from the active set.
    pub registered_at_ms: TimestampMs,
}

impl Provider {
    /// Construct a fresh `Provider` for `RegisterProvider` execution.
    pub fn new(address: SomaAddress, endpoint: String, registered_at_ms: TimestampMs) -> Self {
        Self { address, endpoint, registered_at_ms }
    }
...
mod tests {
    ...
    fn provider_bcs_round_trip() {
        let p = Provider::new(addr(7), "https://example.com".to_string(), 12_345);
    ...
    fn object_provider_helpers() {
        let p = Provider::new(addr(3), "https://prov.test".to_string(), 99);
    ...
    fn set_provider_data_round_trips() {
        let p1 = Provider::new(addr(4), "https://old.example".to_string(), 1);
        let mut obj = Object::new_provider_for_testing(p1);
        let mut p2 = obj.as_provider().unwrap();
        p2.endpoint = "https://new.example".to_string();
        p2.registered_at_ms = 2;
        ...
        assert_eq!(read_back.registered_at_ms, 2);
    }
