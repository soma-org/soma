...
pub(crate) fn parse_end<'a, T: Iterator<Item = Protocol<'a>>>(protocols: &mut T) -> Result<()> {
    if protocols.next().is_none() { Ok(()) } else { Err(eyre!("expected end of multiaddr")) }
}
...
mod test {
    ...
    fn test_to_socket_addr_unsupported_protocol() {
        let multi_addr_dns = Multiaddr(multiaddr!(Dnsaddr("x.soma"), Tcp(10500u16)));
    ...
    fn test_is_loosely_valid_tcp_addr() {
        ...
        assert!(!multi_addr_ipv6.is_loosely_valid_tcp_addr());
        let multi_addr_dns = Multiaddr(multiaddr!(Dnsaddr("x.soma"), Udp(10500u16)));
        assert!(!multi_addr_dns.is_loosely_valid_tcp_addr());
    ...
    fn test_get_hostname_port() {
        ...
        let multi_addr_dns = Multiaddr(multiaddr!(Dns("x.soma"), Tcp(10501u16)));
        assert_eq!(Some("x.soma".to_string()), multi_addr_dns.hostname());
    ...
    fn test_with_zero_ip() {
        ...
        let multi_addr_ip6 =
            Multiaddr(multiaddr!(Ip6([15, 15, 15, 15, 15, 15, 15, 1]), Tcp(10500u16)))
                .with_zero_ip();
        assert_eq!(Some("::".to_string()), multi_addr_ip6.hostname());
        assert_eq!(Some(10500u16), multi_addr_ip4.port());

        let multi_addr_dns = Multiaddr(multiaddr!(Dns("x.soma"), Tcp(10501u16))).with_zero_ip();
    ...
    fn test_with_localhost_ip() {
        ...
        let multi_addr_dns =
            Multiaddr(multiaddr!(Dns("x.soma"), Tcp(10501u16))).with_localhost_ip();
