...
pub struct DagState {
    ...
    // Buffer the last_committed_rounds to be flushed with the
    // next dag state flush. This is okay because we can recover
...
impl DagState {
    ...
    pub fn get_blocks(&self, block_refs: &[BlockRef]) -> Vec<Option<VerifiedBlock>> {
    ...
    pub fn get_last_cached_block_per_authority(&self, end_round: Round) -> Vec<VerifiedBlock> {
    ...
    pub fn last_commit_leader(&self) -> Slot {
        match &self.last_commit {
    ...
    pub fn flush(&mut self) {
        ...
        //     let commit_info = CommitInfo::new(self.last_committed_rounds.clone());
...
mod test {
    ...
    use crate::dag::test_dag::DagBuilder;
    use crate::storage::consensus::{mem_store::MemStore, WriteBatch};
