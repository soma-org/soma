...
use super::universal_committer::universal_committer_builder::UniversalCommitterBuilder;
use super::universal_committer::UniversalCommitter;
use crate::committee::AuthorityIndex;
use crate::consensus::{
    block::{BlockAPI as _, Slot, TestBlock, Transaction, VerifiedBlock},
...
use crate::dag::{
    dag_state::DagState,
    test_dag::{build_dag, build_dag_layer, parse_dag, DagBuilder},
};
use crate::storage::consensus::mem_store::MemStore;
...
async fn direct_commit() {
    ...
    let sequence = test_setup.committer.try_decide(last_decided, None);
...
async fn idempotence() {
    ...
    let second_sequence = committer.try_decide(last_decided, None);
...
async fn no_genesis_commit() {
    ...
    for r in 0..decision_round_wave_1 {
        ...
        let sequence = committer.try_decide(last_committed, None);
        tracing::info!("Commit sequence: {sequence:#?}");
...
async fn direct_skip_missing_leader_block() {
    ...
    let last_committed = Slot::new_for_test(0, 0);
    let sequence = test_setup.committer.try_decide(last_committed, None);
    tracing::info!("Commit sequence: {sequence:#?}");
...
async fn indirect_commit() {
    ...
    let dag_state = Arc::new(RwLock::new(DagState::new(
        ...
        Arc::new(MemStore::new_with_committee(
            dag_builder.context.committee.clone(),
        )),
    )));
    let leader_schedule = Arc::new(LeaderSchedule::new(dag_builder.context.clone(), None));

    dag_builder.print();
    dag_builder.persist_all_blocks(dag_state.clone());

    // Create committer without pipelining and only 1 leader per leader round
    let committer = UniversalCommitterBuilder::new(leader_schedule, dag_state.clone()).build();
...
fn basic_test_setup() -> (Arc<Context>, Arc<RwLock<DagState>>, UniversalCommitter) {
    let _ = tracing_subscriber::fmt::try_init();
