...
pub trait Connection: Send {
    /// Initializes a watermark by either returning the existing watermark or, if the impl supports
    /// it, attempting to create it. Returns `Ok(Some(_))` if a watermark existed or was created by
    /// this impl. Returns `Ok(None)` if a watermark does not exist and this impl does not attempt
    /// to create one. Returns `Err(_)` if the store encountered an error.
    ...
    /// Checks if the store can accept a `chain_id`. Returns `Ok(true)` if the store accepts the
    /// `chain_id` so processing can continue. Returns `Ok(false)` if the store does not accept the
    /// `chain_id`. Returns `Err(_)` if the store encountered an error while trying to determine if
    /// it could accept the `chain_id` (the framework will retry).
    ...
    /// Given a `pipeline_task` (a pipeline name, or `{pipeline}{Store::DELIMITER}{task}`), return
    /// the committer watermark from the `Store`. The indexer fetches this value for each pipeline
    /// added to determine which checkpoint to resume processing from.
    async fn committer_watermark(
        &mut self,
        pipeline_task: &str,
    ) -> anyhow::Result<Option<CommitterWatermark>>;

    /// Upsert the high watermark for the `pipeline_task` as long as it raises the watermark stored
    /// in the database. `checkpoint_hi_inclusive` should not regress; equal or lower writes are
    /// stale and should not update stored state. Returns a boolean indicating whether the
...
pub trait ConcurrentConnection: Connection {
    ...
    /// indexer to determine the new `reader_lo` or inclusive lower bound of available data.
    ...
    /// `pipeline`. The pruner is allowed to prune the region between the returned `pruner_hi`
    ...
    /// Update the pruner watermark only if it raises `pruner_hi`. Returns true if the watermark
...
pub trait ConcurrentStore: for<'c> Store<Connection<'c> = Self::ConcurrentConnection<'c>> {
    ...
    /// Delimiter used to separate pipeline names from task identifiers when reading or writing
    /// the committer watermark.
...
pub struct PrunerWatermark {
    /// `(pruner_timestamp + delay) - current_time`. Positive: wait this long before pruning. Zero
    /// or negative: pruning can begin immediately.
    pub wait_for_ms: i64,

    /// The pruner can delete up to this checkpoint (exclusive).
    pub reader_lo: u64,

    /// The pruner has already deleted up to this checkpoint (exclusive); continue from here.
    pub pruner_hi: u64,
}
...
impl PrunerWatermark {
    ...
    /// no more checkpoints to prune, returns `None`. Otherwise, returns `(from, to_exclusive)` and
...
/// used for tracking a pipeline's watermarks in the store.
