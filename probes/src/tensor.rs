...
pub trait IntoTensorData {
    fn to_tensor_data(self) -> ShardResult<TensorData>;
}
...
fn dtype_to_safetensors(dtype: DType) -> ShardResult<safetensors::Dtype> {
    ...
    match dtype {
        ...
        DType::U16 => Err(ShardError::FailedTypeVerification(
            "U16 dtype not yet supported in safetensors".to_string(),
        )),
        DType::U8 => Ok(Dtype::U8),
        DType::Bool => Ok(Dtype::BOOL),
        DType::QFloat(_) => Err(ShardError::FailedTypeVerification(
            "Quantized tensors not yet supported in safetensors".to_string(),
        )),
    }
}

fn safetensor_dtype_to_burn(dtype: safetensors::Dtype) -> ShardResult<DType> {
    use safetensors::Dtype;
    ...
    match dtype {
        ...
        Dtype::U8 => Ok(DType::U8),
        Dtype::BOOL => Ok(DType::Bool),
        _ => Err(ShardError::FailedTypeVerification(format!(
            "Unsupported dtype: {:?}",
            dtype
        ))),
    }
