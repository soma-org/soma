...
use soma_indexer_alt_metrics::MetricsArgs;
use tracing::info;
use url::Url;

use checkpoint_blob_indexer::CheckpointBlobPipeline;
use checkpoint_blob_indexer::EpochsPipeline;
use checkpoint_blob_indexer::IndexerConfig;

#[derive(Debug, Parser)]
#[command(name = "checkpoint-blob-indexer")]
...
struct Args {
    ...
    /// Write to AWS S3. Provide the bucket name.
...
async fn main() -> anyhow::Result<()> {
    ...
    let metrics_service =
        soma_indexer_alt_metrics::MetricsService::new(args.metrics_args, registry.clone());
