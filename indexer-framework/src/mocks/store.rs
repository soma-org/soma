...
/// Mock connection that holds a reference to the store.
///
/// The phantom lifetime `'c` satisfies the `Connection<'c>` GAT, while the actual store data
/// is accessed through the reference. For `TransactionalStore`, which requires the connection
/// to live as long as `&'c self`, we use the same reference-based approach but construct
/// the connection in a way that satisfies the scoped future lifetime requirements.
...
impl Connection for MockConnection<'_> {
    async fn init_watermark(
        &mut self,
        pipeline_task: &str,
        checkpoint_hi_inclusive_hint: Option<u64>,
        ...
        Ok(Some(InitWatermark {
            checkpoint_hi_inclusive: Some(checkpoint_hi_inclusive),
            reader_lo: Some(default_next_checkpoint),
        }))
    }
