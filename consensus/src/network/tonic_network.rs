...
// Original source: https://github.com/MystenLabs/sui/tree/main/consensus/core/src/network/tonic_network.rs
...
impl NetworkClient for TonicClient {
    ...
    async fn send_block(
        &self,
        peer: AuthorityIndex,
        block: &types::consensus::block::VerifiedBlock,
        timeout: Duration,
    ) -> ConsensusResult<()> {
        let mut client = self.get_client(peer, timeout).await?;
...
type Channel = tower_http::trace::Trace<
...
impl ChannelPool {
    ...
    async fn get_channel(
        ...
        let authority = self
            ...
            .ok_or(ConsensusError::MissingAuthority(peer))?;
...
impl<S: NetworkService> NetworkManager<S> for TonicManager {
    ...
    async fn install_service(&mut self, service: Arc<S>) {
        ...
        let authority = self
            .context
            .committee
            .authority_by_authority_index(self.context.own_index)
            .expect("Own index not present in committee");
