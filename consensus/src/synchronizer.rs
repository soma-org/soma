...
use crate::{
    commit_syncer::CommitVoteMonitor, core_thread::CoreThreadDispatcher, dag_state::DagState,
        ...
        loop {
            tokio::select! {
                Some(blocks_guard) = receiver.recv(), if requests.len() < FETCH_BLOCKS_CONCURRENCY => {
                    ...
                    requests.push(Self::fetch_blocks_request(network_client.clone(), peer_index, blocks_guard, highest_rounds, true, FETCH_REQUEST_TIMEOUT, context.clone(), 1))
                },
                    ...
                    match response {
                        ...
                        Err(_) => {
                            if retries <= MAX_RETRIES {
                                requests.push(Self::fetch_blocks_request(network_client.clone(), peer_index, blocks_guard, highest_rounds, true, FETCH_REQUEST_TIMEOUT, context.clone(), retries))
                            } else {
...
#[cfg(test)]
mod tests {
    use crate::synchronizer::COMMIT_LAG_MULTIPLIER;
