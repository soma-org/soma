...
// Original source: https://github.com/MystenLabs/sui/tree/main/consensus/core/src/block_manager.rs
...
mod tests {
    ...
    async fn suspend_blocks_with_missing_ancestors() {
        ...
        // AND the missing blocks are the parents of the round 2 blocks.
    ...
    async fn accept_blocks_with_causal_history_below_gc_round() {
        ...
        // All those blocks should eventually be returned as accepted.
    ...
    /// Blocks that are attempted to be accepted but are <= gc_round they will be skipped for processing.
    #[tokio::test]
    ...
    /// The test generates blocks for a well connected DAG and feeds them to block manager in random order.
    /// In the end all the blocks should be uniquely accepted and no missing blocks should exist.
    /// We set a high gc_depth value so in this test gc_round will be 0.
    #[tokio::test]
    async fn accept_blocks_unsuspend_children_blocks() {
        ...
        // accepted and no missing blocks should exist.
        for seed in 0..100u8 {
    ...
    #[tokio::test]
    async fn unsuspend_blocks_for_latest_gc_round() {
        // Test with multiple gc_depth values
        for gc_depth in [5u32, 10, 14] {
            ...
            // Pay attention that we start from round 2. Round 1 will always be missing so no matter what we do
            // we can't unsuspend it unless gc_round has advanced to round >= 1.
            ...
            // Randomize the sequence of sending the blocks to block manager.
            for seed in 0..100u8 {
                all_blocks.shuffle(&mut StdRng::from_seed([seed; 32]));
                ...
                // AND also call the try_to_find method with some non existing block refs.
    ...
    async fn test_verify_block_timestamps_and_accept() {
        ...
        // All blocks should be accepted since timestamp verification is done at accept time
