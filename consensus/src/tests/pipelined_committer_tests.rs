...
// Original source: https://github.com/MystenLabs/sui/tree/main/consensus/core/src/tests/pipelined_committer_tests.rs
...
async fn idempotence() {
    ...
    let last_decided = first_sequence[0].slot();
    let sequence = committer.try_decide(last_decided);
...
async fn direct_skip_enough_blame() {
    ...
    let leader_round_pipeline_1_wave_0 = committer.committers[1].leader_round(0);
    let leader_pipeline_1_wave_0 = committer.get_leaders(leader_round_pipeline_1_wave_0)[0];
    let references_round_1 =
        build_dag(context.clone(), dag_state.clone(), None, leader_round_pipeline_1_wave_0);
...
async fn indirect_commit() {
    ...
    let references_without_leader_1: Vec<_> = references_round_1
        ...
        .cloned()
        .filter(|x| {
            x.author != *committer.get_leaders(leader_round_pipeline_1_wave_0).first().unwrap()
        })
        .collect();
    ...
    let connections_with_votes_for_leader_1 = context
        ...
        .collect::<Vec<_>>();
    references_round_3
        .extend(build_dag_layer(connections_with_votes_for_leader_1, dag_state.clone()));
...
async fn undecided() {
    ...
    let connections =
        leader_connection.into_iter().chain(non_leader_connections).collect::<Vec<_>>();
...
async fn test_byzantine_validator() {
    ...
    let references_round_15 =
        build_dag(context.clone(), dag_state.clone(), Some(references_round_14), 15);

    // Ensure B13 is marked as undecided as there is <2f+1 blame and <2f+1 certs
    let last_sequenced = sequence.last().unwrap();
    let last_decided = last_sequenced.slot();
    let sequence = committer.try_decide(last_decided);
    assert!(sequence.is_empty());
