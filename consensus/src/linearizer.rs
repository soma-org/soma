...
// Original source: https://github.com/MystenLabs/sui/tree/main/consensus/core/src/linearizer.rs
// Copyright (c) Mysten Labs, Inc.
// SPDX-License-Identifier: Apache-2.0
#[cfg(test)]
mod tests {
    ...
    async fn test_handle_already_committed() {
        ...
        // Mark the blocks as committed in DagState.
    ...
    async fn test_handle_commit_below_highest_committed_round() {
        ...
        // Then on round 3 the authorities A, B & C will link to block D1. Once the DAG gets committed we should see the block D1 getting committed as well.
        let dag_str = "DAG {
                Round 0 : { 4 },
                Round 1 : { * },
                Round 2 : {
    ...
    async fn test_median_timestamps_by_stake_errors() {
        ...
        // With normalized voting power: each authority has 2500, quorum threshold is 6667
        assert!(err.starts_with("Total stake"), "Expected stake error, got: {err}");
        assert!(err.contains("< quorum threshold"), "Expected quorum error, got: {err}");
    }
}
