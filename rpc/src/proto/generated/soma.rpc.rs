...
///
/// Stage 13m: `coin_type` is now populated. Previously implicit (single
/// native coin); the address-balance accumulator keys per `(address,
/// coin_type)`, so each tx can produce multiple BalanceChange entries
/// for one address — one per coin type touched.
#[non_exhaustive]
...
pub struct BalanceChange {
    ...
    /// Coin denomination this delta applies to (e.g. "SOMA", "USDC").
...
pub struct ChangedObject {
    ...
    /// Stage 14c: populated when output_state is
    /// OUTPUT_OBJECT_STATE_ACCUMULATOR_WRITE_V1. Operation is "Merge"
    /// (deposit) or "Split" (withdraw); amount is the magnitude.
    #[prost(string, optional, tag = "12")]
    pub accumulator_operation: ::core::option::Option<::prost::alloc::string::String>,
    #[prost(uint64, optional, tag = "13")]
    pub accumulator_amount: ::core::option::Option<u64>,
}
...
pub mod changed_object {
    ...
    pub enum OutputObjectState {
        ...
        DoesNotExist = 1,
        ObjectWrite = 2,
        /// Stage 14c: per-tx accumulator delta record (Sui SIP-58 style).
        /// The parent ChangedObject's `object_id` is the canonical
        /// accumulator address. `accumulator_operation` and
        /// `accumulator_amount` carry the delta.
        AccumulatorWriteV1 = 3,
    }
...
pub struct Owner {
    ...
    #[prost(uint64, optional, tag = "3")]
    pub version: ::core::option::Option<u64>,
    /// Stage 14a: kind discriminator for `ACCUMULATOR`. Values
    /// are `BALANCE` or `DELEGATION` (string form) so the wire
    /// representation is forward-compatible if more accumulator
    /// families are added later.
    #[prost(string, optional, tag = "4")]
    pub accumulator_kind: ::core::option::Option<::prost::alloc::string::String>,
}
...
pub mod owner {
    ...
    pub enum OwnerKind {
        ...
        Shared = 2,
        Immutable = 3,
        /// Stage 14a: system-managed accumulator object. The
        /// `accumulator_kind` field discriminates between Balance
        /// (per-(owner, coin_type) account-balance accumulator) and
        /// Delegation (per-(pool_id, staker) F1 row).
        Accumulator = 4,
    }
    impl OwnerKind {
        /// String value of the enum field names used in the ProtoBuf definition.
...
pub mod transaction_kind {
    ...
    pub enum Kind {
        ...
        /// Challenge transactions (deprecated)
        ...
        BridgeWithdraw(super::BridgeWithdraw),
...
pub struct BridgeDeposit {
    ...
    #[prost(bytes = "bytes", optional, tag = "2")]
    pub eth_tx_hash: ::core::option::Option<::prost::bytes::Bytes>,
    #[prost(string, optional, tag = "3")]
    pub recipient: ::core::option::Option<::prost::alloc::string::String>,
    #[prost(uint64, optional, tag = "4")]
    pub amount: ::core::option::Option<u64>,
