...
pub trait AccountKeystore: Send + Sync {
    ...
    async fn remove(&mut self, address: SomaAddress) -> Result<(), anyhow::Error>;
    /// Return an array of `PublicKey`, consisting of every public key in the keystore.
    fn entries(&self) -> Vec<PublicKey>;
    /// Return `SomaKeyPair` for the given address.
    fn export(&self, address: &SomaAddress) -> Result<&SomaKeyPair, anyhow::Error>;
    ...
    /// Return an array of `SomaAddress`, consisting of every address in the keystore.
    fn addresses(&self) -> Vec<SomaAddress> {
        self.entries().iter().map(|k| k.into()).collect()
    }
    /// Return an array of (&SomaAddress, &Alias), consisting of every address and its corresponding alias in the keystore.
    ...
    fn get_alias(&self, address: &SomaAddress) -> Result<String, anyhow::Error>;
...
impl FileBasedKeystore {
    ...
    /// To see Bech32 format encoding, use `soma keytool export $SUI_ADDRESS` where
    /// $SUI_ADDRESS can be found with `soma keytool list`. Or use `soma keytool convert $BASE64_STR`
