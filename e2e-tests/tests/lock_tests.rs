...
async fn test_lock_persists_after_insufficient_gas() {
    ...
    let create_dust_tx = TransactionData::new(
        TransactionKind::Transfer {
            coins: vec![main_coin],
            amounts: Some(dust_amount).map(|a| vec![a]),
            recipients: vec![sender], // Send to self to create a new small coin
        },
        sender,
        vec![funding_gas],
    );
    ...
    let fail_tx = TransactionData::new(
        TransactionKind::Transfer {
            coins: vec![*dust_coin_id], // Use dust coin as the transfer coin AND gas
            amounts: Some(vec![1]),      // Try to transfer 1 shannon
