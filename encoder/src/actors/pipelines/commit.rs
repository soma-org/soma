    ...
    for CommitProcessor<E, O, S>
{
    ...
    async fn process(&self, msg: ActorMessage<Self>) {
        let result: ShardResult<()> = async {
            ...
            let input =
                DownloaderInput::new(peer, address, verified_signed_commit.commit_metadata());
            ...
            let _ = self
                .store
                .add_signed_commit(&shard, &verified_signed_commit.deref().to_owned())?;

            self.shard_tracker
                .add_valid_commit(shard, verified_signed_commit);
