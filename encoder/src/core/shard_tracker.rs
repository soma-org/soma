...
impl<C: EncoderInternalNetworkClient, S: ObjectStorage, P: ProbeClient> ShardTracker<C, S, P> {
    ...
    pub(crate) async fn track_valid_commit_votes(
        ...
        for (encoder, digest) in commit_votes.accepts() {
            ...
            debug!(
                "Evaluating encoder commit votes - encoder: {:?}, accept_count: {:?}, \
                 reject_count: {}, highest: {}, quorum_threshold: {}",
                encoder,
                vote_counts.accept_count().unwrap_or(0_usize),
        ...
        for encoder in rejects {
            ...
            {
                info!(
                    "Rejected encoder commit REJECTED - either reject count >= quorum or can't \
                     reach quorum for encoder: {:?}",
                    encoder
                );
                finalized_encoders.insert(encoder.clone(), Finality::Rejected);
            } else {
        ...
        if total_finalized == shard.size() {
            if total_accepted >= shard.quorum_threshold() as usize {
                ...
                if already_revealing {
                    info!(
                        "Skipping trigger of BroadcastAction::Reveal as reveal phase has already \
                         started"
                    );
                    return Ok(());
                }
            ...
            debug!(
                "Not all commits finalized yet - continuing to collect votes. Total finalized: \
                 {}, shard_size: {}",
                total_finalized,
                shard.size()
            );
