...
from soma_models.v1.torch.modules.encoder import Encoder, EncoderConfig
...
def _build_encoder_weights(seed, num_layers, embedding_dim, hidden_dim):
    generated_tensors = {}
    for layer in range(num_layers):
        lseed = seed + layer
        layer_tensors = {
...
def test_v1_encoder_normal_input():
    ...
    generated_tensors = _build_encoder_weights(seed, num_layers, embedding_dim, hidden_dim)
    serialized_tensors = save(generated_tensors)
    load_safetensor_into(module, serialized_tensors)
    module.eval()
    inputs = torch.tensor(
