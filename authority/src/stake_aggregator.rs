        ...
        match self.data.entry(authority) {
            Entry::Occupied(oc) => {
                return InsertResult::Failed {
                    error: SomaError::StakeAggregatorRepeatedSigner {
                        signer: authority,
...
where
    K: Clone + Ord,
{
    pub fn get_all_unique_values(&self) -> BTreeMap<K, (Vec<AuthorityName>, VotingPower)> {
        self.stake_maps
            .iter()
    ...
    K: Hash + Eq,
{
    ...
    pub fn uncommitted_stake(&self) -> VotingPower {
