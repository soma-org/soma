...
use types::{base::TimestampMs, protocol::Chain};
...
impl ThroughputProfileRanges {
    ...
    pub fn resolve(&self, current_throughput: u64) -> ThroughputProfile {
        ...
        warn!(
            "Could not resolve throughput profile for throughput {} - we shouldn't end up here. \
             Fallback to lowest profile as default.",
            current_throughput
        );
...
impl ConsensusThroughputCalculator {
    pub fn new(observations_window: Option<NonZeroU64>) -> Self {
    ...
    pub fn add_transactions(&self, timestamp_ms: TimestampMs, num_of_transactions: u64) {
        ...
        if let Some((front_ts, transactions)) = inner.observations.front_mut() {
            ...
            if timestamp_secs < *front_ts {
                warn!(
                    "Ignoring observation of transactions:{} as has earlier timestamp than last \
                     observation {}s < {}s",
        ...
        if inner.observations.len() as u64 >= self.observations_window {
            let last_element_ts = if inner.observations.len() as u64 == self.observations_window {
                if let Some(ts) = inner.last_oldest_timestamp {
                    ...
                    warn!(
                        "Skip calculation - we still don't have enough elements to pop the last \
                         observation"
                    );
                    return;
                }
            ...
            if period > 0 {
                ...
                warn!(
                    "Skip calculating throughput as time period is {}. This is very unlikely to \
                     happen, should investigate.",
                    period
                );
            }
        }
...
mod tests {
    use super::*;
    use crate::throughput::Level::{High, Low};
    ...
    pub fn test_consensus_throughput_calculator() {
        ...
        let calculator = ConsensusThroughputCalculator::new(Some(max_observation_points));
    ...
    pub fn test_consensus_throughput_profiler() {
        ...
        let calculator = Arc::new(ConsensusThroughputCalculator::new(Some(
