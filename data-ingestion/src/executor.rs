use crate::Worker;
use crate::reader::{CommitReader, ReaderOptions};
use anyhow::Result;
use futures::Future;
...
pub static MAX_COMMITS_IN_PROGRESS: Lazy<usize> = Lazy::new(|| {
    std::env::var("MAX_COMMITS_IN_PROGRESS")
        .ok()
...
/// Worker pool for processing commits
...
impl<W: Worker + 'static> WorkerPool<W> {
    ...
    pub async fn run(
        self,
        watermark: CommitIndex,
        mut archive_receiver: mpsc::Receiver<Arc<CommitArchiveData>>,
        executor_progress_sender: mpsc::Sender<(String, CommitIndex)>,
    ) {
        info!(
            "Starting worker pool {} with concurrency {}. Starting at commit {}",
            self.task_name, self.concurrency, watermark
        );

        // Change: Remove Result from the progress channel type
        let (progress_sender, mut progress_receiver) =
            mpsc::channel::<(usize, CommitIndex)>(*MAX_COMMITS_IN_PROGRESS);
        let mut workers = vec![];
        let mut idle: BTreeSet<_> = (0..self.concurrency).collect();
        let mut pending_archives = std::collections::VecDeque::new();
        let mut current_commit = watermark;

        // Spawn child workers
        for worker_id in 0..self.concurrency {
            let (worker_sender, mut worker_recv) =
                mpsc::channel::<(CommitIndex, Arc<CommitArchiveData>)>(*MAX_COMMITS_IN_PROGRESS);
            ...
            let handle = tokio::spawn(async move {
                loop {
                    tokio::select! {
                        ...
                        Some((commit_index, archive_data)) = worker_recv.recv() => {
                            info!("Worker {} processing commit {} for {}", worker_id, commit_index, task_name);
                            ...
                            let result = backoff::future::retry(backoff, || async {
                                worker
                                    .process_commit_archive(&archive_data)
                                    .await
                                    .map_err(|err| {
                                        info!("Transient error for commit {}: {:?}", commit_index, err);
                            ...
                            match result {
                                Ok(_res) => {
                                    info!("Worker {} finished commit {} in {:?}", worker_id, commit_index, start.elapsed());
                                }
                                Err(e) => {
                                    tracing::error!("Worker {} failed to process commit {}: {}", worker_id, commit_index, e);
                                    // Continue anyway - we still mark this commit as processed
                                }
                            }

                            // Always send progress, regardless of success/failure
                            // Change: Just send worker_id and commit_index, no result
                            if progress_sender.send((worker_id, commit_index)).await.is_err() {
                                break;
                            }
                        }
                    }
        ...
        loop {
            tokio::select! {
                ...
                Some((worker_id, commit_index)) = progress_receiver.recv() => {
                    idle.insert(worker_id);

                    // Send progress to executor
                    if executor_progress_sender.send((self.task_name.clone(), commit_index)).await.is_err() {
                        break;
                    }
                    ...
                    while !pending_archives.is_empty() && !idle.is_empty() {
                        ...
                        if workers[worker_id].0.send((current_commit, archive)).await.is_err() {
                            break;
                        }
                        current_commit += 1;
                    }
...
pub struct IndexerExecutor {
    ...
    pool_senders: Vec<mpsc::Sender<Arc<CommitArchiveData>>>,
    progress_sender: mpsc::Sender<(String, CommitIndex)>,
    progress_receiver: mpsc::Receiver<(String, CommitIndex)>,
    current_commit: CommitIndex,
}

impl IndexerExecutor {
    pub fn new(starting_commit: CommitIndex, num_workers: usize) -> Self {
        let (progress_sender, progress_receiver) =
            mpsc::channel(MAX_COMMITS_IN_PROGRESS.saturating_mul(num_workers));
    ...
    pub async fn register<W: Worker + 'static>(&mut self, pool: WorkerPool<W>) -> Result<()> {
        let (sender, receiver) = mpsc::channel(*MAX_COMMITS_IN_PROGRESS);
    ...
    pub async fn run(
        ...
        let mut last_commit = self.current_commit;

        loop {
            tokio::select! {
                _ = &mut exit_receiver => break,

                Some((task_name, commit_index)) = self.progress_receiver.recv() => {
                    info!("Task {} processed commit {}", task_name, commit_index);
                    gc_sender.send(commit_index).await?;
                    last_commit = last_commit.max(commit_index);
                    total_processed += 1;

                    if let Some(limit) = reader_options.upper_limit {
                        if commit_index >= limit && self.pool_senders.len() == 1 {
                            break;
                        }
                    }
                }

                Some(archive_data) = commit_recv.recv() => {
...
pub async fn setup_single_workflow_with_options<W: Worker + 'static>(
    ...
    let mut executor = IndexerExecutor::new(initial_commit_index, 1);
