...
mod tests {
    ...
    fn verify_allowall() {
        ...
        let random_cert_bob = SelfSignedCertificate::new(allowed.private(), SERVER_NAME);
        let random_cert_alice = SelfSignedCertificate::new(disallowed.private(), SERVER_NAME);

        let verifier = ClientCertVerifier::new(AllowAll, SERVER_NAME.to_string());
    ...
    fn verify_server_cert() {
        ...
        let verifier = ServerCertVerifier::new(allowed_public_key, SERVER_NAME.to_string());
    ...
    fn verify_hashset() {
        ...
        let allowed_cert = SelfSignedCertificate::new(allowed.private(), SERVER_NAME);

        let disallowed_cert = SelfSignedCertificate::new(disallowed.private(), SERVER_NAME);

        let allowlist = AllowPublicKeys::new(allowed_public_keys);
        let verifier = ClientCertVerifier::new(allowlist.clone(), SERVER_NAME.to_string());
    ...
    fn invalid_server_name() {
        ...
        let client_verifier = ClientCertVerifier::new(allowlist.clone(), SERVER_NAME.to_string());
        ...
        let server_verifier = ServerCertVerifier::new(public_key, SERVER_NAME.to_string());
    ...
    async fn axum_acceptor() {
        ...
        let client_certificate = SelfSignedCertificate::new(client_keypair.private(), SERVER_NAME);
