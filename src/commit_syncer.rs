...
//! quorum certified commits themselves can be trusted to be sent directly, but for
...
impl<C: NetworkClient> CommitSyncer<C> {
    pub(crate) fn new(
        ...
        let schedule_task = tokio::spawn(Self::schedule_loop(inner, fetch_state, rx_shutdown));
        CommitSyncer {
            schedule_task,
            tx_shutdown,
