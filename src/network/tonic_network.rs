...
use crate::{
    block::{BlockRef, VerifiedBlock}, commit::CommitRange, context::Context, error::{ConsensusError, ConsensusResult}, network::{
        multiaddr::{Multiaddr, Protocol}, tls::{create_rustls_client_config, create_rustls_server_config, public_key_from_certificate}, tonic_gen::consensus_service_server::ConsensusServiceServer
    }, sync_utils::notify_once::NotifyOnce, AuthorityIndex, CommitIndex, NetworkKeyPair, NetworkPublicKey, Round
};
...
impl<S: NetworkService> NetworkManager<S> for TonicManager {
    ...
    async fn install_service(&mut self, service: Arc<S>) {
        ...
        let listener = loop {
            ...
            // TODO: Handle madsim TCPListener case
