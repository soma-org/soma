...
impl TransactionClient {
    pub(crate) fn new(context: Arc<Context>) -> (Self, Receiver<TransactionsGuard>) {
        ...
        (
            Self {
                sender,
                max_transaction_size: context.parameters.consensus_max_transaction_size_bytes,
            },
            receiver,
        )
    }
...
/// `TransactionVerifier` implementation is supplied to validate transactions in a block,
...
mod tests {
    ...
    async fn basic_submit_and_consume() {
        let context = Arc::new(Context::new_for_test(4).0.with_parameters(
