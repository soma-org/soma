...
async fn test_checkpoints_by_digest_process() {
    let checkpoint = TestCheckpointBuilder::new(1).build();
    let entries = CheckpointsByDigestPipeline
        .process(&Arc::new(checkpoint))
        .await
        .unwrap();
    assert_eq!(
        entries.len(),
        1,
        "should produce exactly 1 checkpoint-by-digest entry"
    );
}
...
async fn test_transactions_process() {
    ...
    assert_eq!(
        entries.len(),
        1,
        "should produce 1 transaction entry for 1 transfer"
    );
}
...
async fn test_objects_process() {
    ...
    assert!(
        entries.len() >= 2,
        "should produce entries for output objects, got {}",
        entries.len()
    );
}
...
async fn test_epoch_start_genesis() {
    ...
    assert_eq!(
        entries.len(),
        1,
        "genesis checkpoint should produce 1 epoch-start entry"
    );
}
...
async fn test_epoch_start_non_epoch() {
    ...
    assert_eq!(
        entries.len(),
        0,
        "non-epoch checkpoint should produce no epoch-start entries"
    );
}
...
async fn test_epoch_end_with_boundary() {
    ...
    assert_eq!(
        entries.len(),
        1,
        "end-of-epoch checkpoint should produce 1 epoch-end entry"
    );
}
...
async fn test_epoch_end_without_boundary() {
    ...
    assert_eq!(
        entries.len(),
        0,
        "normal checkpoint should produce no epoch-end entries"
    );
}
...
async fn test_soma_targets() {
    ...
    assert_eq!(
        entries.len(),
        1,
        "should produce 1 target entry for 1 target"
    );
}
...
async fn test_soma_models_genesis() {
    ...
    let checkpoint = TestCheckpointBuilder::new(0)
        .with_genesis_system_state(default_test_system_state())
        .build();
    let entries = SomaModelsPipeline
        .process(&Arc::new(checkpoint))
        .await
        .unwrap();
    assert_eq!(
        entries.len(),
        0,
        "default system state has no models, so 0 entries expected"
    );
}
...
async fn test_soma_models_non_epoch() {
    ...
    assert_eq!(
        entries.len(),
        0,
        "non-epoch checkpoint should produce no model entries"
    );
}
...
async fn test_soma_rewards() {
    ...
    assert_eq!(
        entries.len(),
        1,
        "should produce 1 reward entry for 1 claim"
    );
}
